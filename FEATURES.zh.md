...
模型目录把客户端看到的 exposed model 映射到真实上游模型 ID，控制模型是否对外暴露，并可指定上游 Chat Completions、Responses 或 Anthropic Messages。`upstream_api` 优先；`vendor=OpenAI` / `Azure OpenAI` 与 `gpt*`/o-series 走 Responses；Anthropic/Claude/Opus/Haiku/Sonnet 走 Messages；Gemini、Microsoft MAI、Grok/xAI 走 Chat Completions；其他模型按下游请求协议选择。
...
flowchart LR
  ...
  F -->|"yes"| G["发送 upstream model + upstream_api"]
...
`upstream_api` 可选，支持 `chat_completions`、`responses` 和 `anthropic_messages`；未填写时会先归一化 `vendor`，再从 `upstream`、`name`、`exposed` 推断模型家族：`gpt*`/o-series 走 Responses，Anthropic/Claude/Opus/Haiku/Sonnet 走 Messages，Gemini、MAI、Grok/xAI 走 Chat Completions，其余模型按下游请求协议选择。要回滚某个 Anthropic 模型，可显式设置 `upstream_api=chat_completions`。
...
| `PROVIDER` | VM deploy 默认 `copilot`；本地开发 compose 默认 `fake` 用于 smoke test |
