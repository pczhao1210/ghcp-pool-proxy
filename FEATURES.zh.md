...
模型目录把客户端看到的 exposed model 映射到真实上游模型 ID，控制模型是否对外暴露，并可指定该模型走上游 Chat Completions 还是 Responses API。GitHub Copilot 上游不是全局默认 Responses：`upstream_api` 优先；`vendor=OpenAI` / `Azure OpenAI` 与 `gpt*`/o-series 走 Responses；Gemini、Anthropic/Claude/Opus/Haiku/Sonnet、Microsoft MAI、Grok/xAI 等非 OpenAI 家族走 Chat Completions；其他模型按下游请求协议选择。
...
`upstream_api` 可选，支持 `chat_completions` 和 `responses`；未填写时会先归一化 `vendor`，再从 `upstream`、`name`、`exposed` 推断模型家族：`gpt*`/o-series 走 Responses，Gemini、Anthropic/Claude/Opus/Haiku/Sonnet、MAI、Grok/xAI 走 Chat Completions，其余模型按下游请求协议选择。
