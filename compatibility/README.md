...
- Claude Code `2.1.225`, `2.1.226`, and `2.1.233` share one `claude-sonnet-4-20250514 / anthropic_messages` runtime contract, but all three are `unsupported`: after an initial Sonnet turn, their observed `--resume` request selects `claude-opus-5`. The support catalog and embedded conversion matrix now contain a static Opus 5 entry and reference profile, but the compatibility matrix still has no matching Claude Code runtime contract, pool, entitlement evidence, fixed-CLI workflow evidence, or release attestation. It therefore fails closed rather than claiming the Sonnet contract. `count_tokens` remains unsupported.
