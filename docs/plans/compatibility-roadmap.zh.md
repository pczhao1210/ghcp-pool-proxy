...
- 状态：Phase 7 后维护修复已实现。Cherry Studio 使用 `/v1/messages + stream:true` 调用 `gpt-5.4 -> responses` 时显示错误名称 `null`、错误信息 `[object Object]`。Gateway 日志以 request `88cb741cb753a265525959bb68585442`、trace `770d02a61bf06e0635ece487eb46172a` 精确定位到上游在 final text 前输出 Responses reasoning item，旧 validator 以 `$.item.type: reasoning_item_not_representable` 中止流；客户端因此看似收到空响应，实际收到的是 Anthropic SSE error object。
- 修复：schema 3 模型 profile 新增 `response_policy.anthropic_messages_reasoning`。GPT-5.4 的 direct-probe profile 声明 `omit`，只在 `responses -> anthropic_messages` 清除 reasoning output item、summary/delta 和非流 canonical reasoning，同时保留 final text、function tool、usage 与成功终态；GPT-5.5 与三个 GPT-5.6 profile 显式声明 `reject`，未知 profile 也继续 fail closed。选择依据是模型目录解析后的精确 upstream ID，不按名称前缀或数值版本猜测。
- 性能与安全边界：流式每事件只做已绑定 profile 的常量策略判断与 reasoning 类型检查，非流只对已解析 output item 做一次稳定过滤；不增加网络、重试、探测、存储读取或额外序列化。该模板不输出私有 reasoning，也不放宽 refusal、annotation、logprobs、hosted tool、namespace tool、Anthropic signature 或普通未知 Responses 语义。
- 验证：协议红灯覆盖 reasoning item、reasoning delta、final text 与默认严格 profile；非流回归覆盖 reasoning item/字段过滤且保留 message item；Gateway HTTP SSE 红灯重放实际顺序并确认最终下游包含 `final answer`、`message_stop`，不包含 private reasoning、`reasoning_item_not_representable` 或 `event:error`。`internal/protocol`、`internal/modelcatalog` 与 `internal/api/gateway` 完整 package 测试通过。
- 下一最小步骤：重建本地 Gateway 后，以相同 Cherry Studio 会话重放 GPT-5.4，并按新 request/trace ID 确认日志不再出现 `reasoning_item_not_representable`；随后对 GPT-5.5 与一个 GPT-5.6 模型做对照，确保它们未进入 5.4 模板。

## 2026-08-22 GPT Chat -> Responses 历史 reasoning 归一

- 状态：Phase 7 后维护修复已实现。Cherry Studio 通过 `/v1/chat/completions` 调用 GPT-5.4/5.5/5.6 均在 Provider dispatch 前返回 400；附件 request `e9c9d0bc6286e9b8a50ef7472772baaa`、trace `cedb033e766b4e4f7880e21f5296b8f2` 精确报 `openai_chat->responses` 在 `$.messages[6].reasoning_content` 出现 `reasoning_not_representable`。请求侧首修后，GPT-5.4 request `e911e0ec4899a42efc3f54eb3092fdcb`、GPT-5.5 request `670c5c3ebd423e99ecb5645d905a2a90` 与 GPT-5.6 Luna request `5daeca7eaa2492d94f042b228d734b1c` 均已进入 Provider，但上游在 final text 前返回 reasoning item，旧 Responses -> Chat validator 又以 `$.item.type: reasoning_item_not_representable` 写出 stream error。同一客户端的 Messages 与 Responses 入口已由用户确认正常，冻结范围因此只覆盖 Chat source 请求和 Chat target 响应策略。
- 修复：schema 3 profile 新增 `request_policy.openai_chat_reasoning_history`。GPT-5.4、GPT-5.5 与 GPT-5.6 Luna/Sol/Terra 五个精确 Responses profile 声明 `omit`：Gateway 在校验与 Provider wire 构造前清除非空 canonical assistant reasoning，保留对应 role、正文、工具调用和消息顺序，并记录原始 `$.messages[i].reasoning_content` path；空字符串不产生无意义诊断。未知/未声明 profile 继续在同一路径 fail closed。
- Chat effort：附件顶层 `reasoning_effort:"high"` 属于 Chat 方言。已绑定 Responses reasoning profile 会将其投影为 `reasoning:{"effort":"high"}` 并删除原字段；合法档位仍由目标 profile 决定，`max` 在 GPT-5.4/5.5 降为 `xhigh`，GPT-5.6 保留 `max`。类型非法、值非法或没有 profile 时保留源字段并由 validator 在 `$.reasoning_effort` 拒绝，不能盲传到 Responses 上游。
- Chat response：五个目标 profile 同时声明 `response_policy.openai_chat_reasoning:"omit"`，只在 `responses -> openai_chat` 省略 reasoning item/summary/delta，保留 final text、function tool、usage 与正常 `[DONE]`。这不改变 Messages 目标策略：GPT-5.4 仍为 `omit`，GPT-5.5/5.6 仍为 `reject`；未授权 profile 也继续拒绝 Responses reasoning。
- 性能与安全边界：请求热路径只遍历已解析 message slice 一次并做常量 profile/level 判断，响应流每事件只做常量目标策略与 reasoning 类型检查；不增加网络、重试、探测、数据库读取、缓冲或序列化轮次。省略的是无法在目标协议中安全回放的 unsigned reasoning，不删除 assistant/final 正文；refusal、audio、message name、hosted tool、annotation、logprobs 和其它不可表示扩展仍严格拒绝。Messages/Responses source parser、GPT-5.4 Messages 响应模板和原生协议路径均未改变。
- 验证：协议红灯覆盖非空/空历史 reasoning、正文保留、normalized path、严格 profile 反例、Chat effort 投影，以及 Chat 目标 reasoning item/delta/non-stream 清除；版本对照锁定 GPT-5.4/5.5 `max -> xhigh`、GPT-5.6 `max -> max`。Gateway HTTP 回归使用 `messages[6].reasoning_content` 和实际上游事件顺序逐个验证五个模型包含 final text 与 `[DONE]`，且无 private reasoning/error；Provider wire 回归确认最终 JSON 只含 `reasoning.effort`，不含 `reasoning_effort` 或私有历史文本。高级语义六方向请求/响应/流拒绝矩阵分别改用仍不可表示的 `refusal` 与 `web_search_call`，继续保持 6/6 fail closed；`gpt-5.4-mini` 锁定未授权 profile 的 reasoning 响应仍返回 502。`go test ./... -count=1` 通过；重建本地 Gateway 后三条真实 Chat 探针均为 HTTP 200、7 个 chunk、非空 content delta、正常 `[DONE]`、零 error：GPT-5.4 request `8903cb518d2b3f2609e373a7f6aba74b`、GPT-5.5 request `19ffd5f0ff28c90321850019f20588cf`、GPT-5.6 Luna request `0501c996f316ff993ee8a2b5e262c5e4`，对应日志均无请求/响应侧 reasoning compatibility 错误。
- 下一最小步骤：使用附件中的完整 Cherry Studio 历史与工具集重放 GPT-5.5，并对 GPT-5.4、GPT-5.6 做同会话对照；若仍失败，只按新 request/trace ID 检查工具或其它高级语义，不再重开已通过的 reasoning 路径。

## 2026-08-22 Claude Chat -> Messages 历史 reasoning 泛化

- 状态：Phase 7 后维护修复已实现。Cherry Studio 通过 `/v1/chat/completions` 调用 Haiku 4.5、Sonnet 5 与 Opus 4.8 时，旧 Chat assistant 历史中的非空 `reasoning_content` 在 Provider dispatch 前被 `openai_chat->anthropic_messages` validator 以 `reasoning_not_representable` 拒绝；空字符串不触发。三模型错误语义相同，Sonnet 5 的 `adaptive_by_default` 只影响当前轮 thinking 生成，不改变 unsigned 历史字段的可表示性。
- 修复：仅在 source=`openai_chat`、target=`anthropic_messages` 且模型目录解析后的真实 upstream ID 以 `claude-` 开头时，省略非空 canonical `Message.Reasoning`，记录原始 `$.messages[i].reasoning_content` path，并保留 role、正文、工具调用与消息顺序。该规则不依赖 conversion profile，因此未来进入动态 catalog 的 `claude-*` Sonnet/Opus/Haiku 型号即使尚无 profile，也不会因同一字段再次返回 400；非 `claude-*` Anthropic upstream、Chat -> Responses 和所有响应方向保持原有 profile/fail-closed 行为。
- 边界：该 fallback 不把 unsigned Chat reasoning 伪造成 Anthropic signed thinking，不改变 Sonnet adaptive 默认、Messages thinking 参数、响应 thinking/signature、工具或 `reasoning_effort` 的既有方向策略。热路径仅复用一次 message slice 遍历和常量前缀判断，无网络、数据库、重试、缓冲或额外序列化。
- 验证：协议反例使用无 conversion profile 的 `claude-future-6.0` 通过，并锁定非 Claude Anthropic 模型仍在原路径拒绝；Gateway 回归确认无 profile 的 Haiku 4.5 与保留各自 thinking profile 的 Sonnet 5/Opus 4.8 均删除 `messages[6].reasoning_content`、保留正文并正常结束 Chat SSE。模型级临时 request policy 已移除，避免测试被精确 profile 掩盖。`go test ./... -count=1` 通过；重建本地 Gateway 后，Haiku request `0885e87920cbd32cf772a5b9625218e3`、Sonnet request `738174685c2546686de2ebc044114863`、Opus request `62f23b4ebc1347fc0991344f0cd3d609` 均为 HTTP 200、有非空 content delta、正常 `[DONE]`、零 error，日志只记录正常 `openai_chat -> anthropic_messages` dispatch 且无 `reasoning_not_representable`。
...
- 修复：不修改通用 policy/validator，仅在两份精确模型 profile 增加观察到的值映射：Sonnet 5 `low -> xhigh`，Opus 4.8 `low -> medium`。现有 `high -> xhigh`、原生 `xhigh/max` 和 Opus 原生 `medium` 保持不变；不新增未报错的其它映射，也不推广到 Opus 5、Sonnet 4.x 或未知 Claude 模型。
- 验证：新增原生 Messages HTTP 红灯逐模型锁定映射结果，并与既有 `high` 回归共同通过；Chat 冻结门 `TestChatToResponsesNormalizesCherryReasoningHistoryByTargetProfile`、`TestChatToMessagesNormalizesHaikuReasoningHistory`、`TestChatToMessagesNormalizesSonnetAndOpusReasoningHistory` 全部通过，`go test ./... -count=1` 通过。重建本地 Gateway 后，Sonnet request `c51aad0524757ccfb6b5324c2e1149e9` 与 Opus request `6645832ff3faedb9f15b1b2d0fed55b0` 均为 HTTP 200、有非空 text delta、正常 `message_stop`、零 error，日志无 `provider_thinking_effort_not_supported`。
- 下一最小步骤：使用附件中的完整签名 thinking 历史与工具集分别重放 Sonnet 5、Opus 4.8；若出现新的上游 path/kind，按该精确语义另开切片，不解冻 Chat 入口或扩展 effort 映射集合。
...
- 修复：新增响应方向 policy，仅在 `chat_completions -> anthropic_messages` 清除 unsigned Chat reasoning text/delta，再执行原有 semantic validator 和 writer；final text、function tool、usage 与成功终态保持不变，stream 正常结束于 `message_stop`。该策略不会把 Chat reasoning 伪造成无签名 Anthropic thinking，也不放宽未获精确模型 profile 授权的 Responses reasoning item、Anthropic thinking/signature、refusal、logprobs 或 audio；绕过 policy 直接调用 validator 时仍 fail closed。
...
- 安全边界：bridge 只接受专用前缀、严格无未知字段的单个 JSON block，以及非空 Anthropic signature 或 redacted data；最大解码体为 8 MiB。畸形/未知前缀、无签名 block、无法关联到后续 function call、普通 Responses reasoning item和孤立 signature继续 fail closed。base64只用于 opaque transport，不提供保密性；正文、thinking、signature和 envelope值不进入日志。最终 signature仍由 Anthropic/Copilot验证。
- 性能边界：仅 signed thinking block执行一次 JSON marshal/base64，续轮执行一次有界 decode和 source-item/message线性扫描；流式状态仅保存当前 thinking block并在请求结束释放。不增加网络、重试、数据库、Redis或持久化状态。
- 验证：红灯在 `response.failed` 前无任何 function call；修复后首轮 Responses SSE含 reasoning `encrypted_content`、function call和正常 `response.completed`。两轮 Gateway黑盒确认第二轮恢复 `[thinking(signature), tool_use] -> tool_result`并得到 final answer。codec malformed/unsigned、未知 bridge、无签名零泄漏、Sonnet route-local adaptive/disabled、sampling控制删除和 Anthropic wire顺序测试通过；新增隔离回归确认原生 Sonnet 5 Messages既不注入默认 thinking也原样输出 thinking/signature/text，Messages -> Responses正常输出非空 text与 `message_stop`且两条路径都没有 bridge授权/前缀。`go test ./... -count=1`与相关编辑器诊断通过；使用 build `2026.08.18.1`和仓库外固定 CLI manifest运行提交级门禁，lint、全仓 race、Dashboard 26 tests/build、compat matrix/fake collector、release/start workflow、VM deploy、Kubernetes/cluster与 Azure Bicep静态检查全部通过。
- 未满足门禁：尚未构建/部署新镜像，也未用 Cherry Studio真实重放 Sonnet 5工具循环；不可变 release attestation、Redis Cluster与实际 Kubernetes集群未运行。
- 下一最小步骤：部署新 Gateway，再用截图中的同一网页搜索分别重放 Sonnet 5与 Opus 4.8；Sonnet应显示 tool call并在工具结果后产生 final text，且不出现 `reasoning_item_not_representable` / `thinking_signature_not_representable`。
...
- 未满足门禁：当前账号无 Opus 5，未执行 Copilot `/models` runtime binding、首轮 text/tool、signed thinking、工具结果续轮或 explicit off/xhigh/max对照探针；因此该判断是 reference-backed candidate，不提升兼容等级，也不设置 `adaptive_by_default`。
...
- 严格边界：普通 Responses reasoning 与 Anthropic thinking signature 继续 fail closed；唯一例外是上述带 Gateway专用前缀、严格校验并关联到 function call的 signed-thinking工具历史 bridge。refusal 跨协议的非流/流式 writer 尚未形成完整对称生命周期，也继续 fail closed。孤立/不完整工具回合不做全局静默删除；Codex function-tools 尚未进入 matrix 合同。`/v1/messages/count_tokens`、`/v1/responses/compact`、WebSocket 与 image tool-result 映射保持 no-go，不能用近似实现冒充原生能力。
