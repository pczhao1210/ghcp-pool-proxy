...
- 状态：Phase 7 后维护修复已实现。第一处失败是 Messages -> Chat 的 `$.system[0]: system_block_not_representable`：方向级 validator 错把纯文本 system block 的合法 `cache_control` 当作不可表达语义；现在 Chat/Responses 投影允许丢弃该 metadata，非文本 system block 仍 fail closed，原生 Messages 保持 typed cache-control 透传。部署后 `gpt-5.6-luna` 的 Messages -> Responses 已到达 Copilot，但稳定返回 `400 invalid_request`；对应 Cherry 请求带 `temperature=1`、`top_p=1`，而引入真实 Phase 7 evidence 的提交 `c3cb160e` 已明确要求 Responses probe 不发送不兼容的 `temperature`。普通 Responses builder 现统一丢弃 `temperature`，保留 `top_p` 和其它参数。此前真实 probe 已证明 `max_tokens=128000` 可接受，因此不实施 catalog clamp。
...
- 验证：新增协议红灯先精确复现 Messages -> Chat 400，修复后 Chat/Messages/Responses 三目标均通过；完整 Cherry 风格流式 HTTP 回归覆盖 system `ephemeral`、message `"[Circular]"`、tool cache-control、`eager_input_streaming`、`max_tokens=128000`，三个目标子测试均到达正确 Provider API 并正常结束。新 Provider 反例先捕获 Messages -> Responses body 同时包含 `temperature`/`top_p`，修复后只删除 `temperature`；聚焦测试和 `go test ./internal/provider/copilot -count=1` 通过，改动 Go 文件编辑器诊断为零。目标日志记录的上游失败为 HTTP `400`、type `invalid_request`、body length `110`、SHA-256 `7aa2a08a6aed9fd3e74698c3cb3e94e110b6921de7a63af3229fb8eb47a7d3ab`。示例 MCP 工具名实际未超过 64 字节，故未提交无证据的工具名改写。
- 未运行检查：尚未部署新 Gateway 并用同一 Cherry 请求验证去除 `temperature` 后的真实 Copilot 结果；未运行完整 `make validate`、release、Redis Cluster、Kubernetes 或目标 VM 门禁，当前 dirty 工作树结果不构成 release evidence。
- 下一最小步骤：发布新 Gateway，重放 request `d60faabd2cc8b9405e97a434f6a4365d` 对应的 `gpt-5.6-luna` 请求，并确认 dispatch body 不再包含 `temperature` 且上游不再返回同一 400 hash。若仍失败，只对新的 status/type/body hash 建立一个反例；下一候选是单独验证 `top_p`，不同时修改工具 schema、数量或模型 limits。
