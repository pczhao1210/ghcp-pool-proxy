...
- 模型差异：CC Switch 的可复现合同明确 `adaptive_thinking_is_default("claude-sonnet-5")`，Responses -> Anthropic 测试期望 Sonnet 5 自动生成 `thinking.type=adaptive`，而 Opus 4.8 同请求不生成 thinking。内嵌模型转换矩阵因此新增显式 `adaptive_by_default`，只为 Sonnet 5 开启，并把 `disabled` 声明为合法类型；不从模型名前缀或单纯 `types:[adaptive]` 推断默认行为。
- 修复：模型转换 matrix revision `2026-08-22.3` 只为 Sonnet 5 声明 adaptive default。首轮请求按 profile投影 adaptive thinking，显式 Responses effort 按 `xhigh/max` 钳位，并删除 thinking 模式不接受的 `temperature/top_p`。Anthropic 返回完整 signed thinking 或 redacted-thinking block 时，Gateway 把严格字段集合 JSON 以 `ghcp-anthropic-thinking-v1:` 前缀和 base64url包装进 Responses `reasoning.encrypted_content`，summary保留可见 thinking text；工具续轮收到该精确 envelope 后，在匹配 function call 的 assistant turn 中恢复原 thinking block并置于 `tool_use` 前。缺少可恢复 signed history 的 Sonnet工具续轮显式发送 `thinking.type=disabled`，避免非法 interleaved-thinking history。Opus 4.8 维持不默认启用 thinking。
...
- 验证：红灯在 `response.failed` 前无任何 function call；修复后首轮 Responses SSE含 reasoning `encrypted_content`、function call和正常 `response.completed`。两轮 Gateway 黑盒从首轮 SSE提取 opaque state，按 Responses reasoning/function_call/output回传，确认第二轮恢复 `[thinking(signature), tool_use] -> tool_result` 并得到 final answer。codec malformed/unsigned反例、未知 bridge request拒绝、无签名流零 thinking泄漏、非流 response投影、Sonnet adaptive default/缺历史 disabled、Opus不默认 adaptive、sampling控制删除和最终 Anthropic wire顺序测试均通过。`go test ./... -count=1` 与相关编辑器诊断通过；使用 build `2026.08.18.1` 和仓库外固定 CLI manifest 运行提交级门禁，lint、全仓 race、Dashboard 26 tests/build、compat matrix/fake collector、release/start workflow、VM deploy、Kubernetes/cluster与 Azure Bicep静态检查全部通过。
...
- 运行行为：无显式 Responses reasoning effort时不主动给 Opus 5开启 thinking；显式 `xhigh/max` 时投影为 Anthropic adaptive thinking。若实际 Opus 5返回 signed thinking，现有 opaque bridge与模型无关，能按 Sonnet 5同一机制保留工具续轮状态；普通/未知 reasoning仍 fail closed。
