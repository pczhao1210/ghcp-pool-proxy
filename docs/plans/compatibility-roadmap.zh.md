...
- 最新公网反例：公网 build `2026.08.18.1`、revision `8c5b8f2bf230158e721014bcd3e3e99e57890b34` 已同时删除 Responses `temperature` 与 `top_p`。同一 profile、账号和 `gpt-5.6-luna` 下，原生 Responses 最小非流式请求返回 `200/OK`，Messages -> Responses 最小非流式和流式分别返回 `502` 与开流后的 `stream response unavailable`，Chat -> Responses 最小非流式也返回 `502`；完整 Cherry 形状不是必要条件。两种转换共享的 Responses body 分支此前把 canonical message 序列化为缺少核心 identity 的 `{role,content}`，而原生分支输出 `{type:"message",role,content}`。Provider builder 现为 Chat/Messages 转换统一补 `type:"message"`，并以双来源 body 回归固定该合同。
- 未运行检查：目标 VM 的 SSH 端口不可达，尚未按公网 request ID 取得安全 `path/kind` 日志；新 `type:"message"` 修复尚未构建或部署，因此尚未完成真实 Copilot 修复后重放，也未运行完整 `make validate`、release、Redis Cluster 或 Kubernetes 门禁。公网反例与 dirty 工作树聚焦测试不构成 release evidence。
- 下一最小步骤：构建并部署包含 converted Responses message identity 修复的 Gateway，再以相同 profile 重放 Messages -> Responses 最小非流式、最小流式和完整 Cherry 形状请求；预期三者均不再出现当前 502/stream error。若仍失败，只按新 request ID 的安全 Provider 日志建立下一个最小反例；Sonnet 原生 Messages 工具 schema 调查继续保持独立。
