...
| Responses to Messages | message/text/image and function call/output items; gateway-prefixed signed Anthropic thinking bridge items | the preceding Responses-only semantics plus Responses Lite/additional tools, unrecognized reasoning items, and refusal deltas | Responses item/lifecycle identity is omitted; recognized bridge state is restored before the matching assistant tool-use turn |
...
| Messages to Responses | text/image and ordinary text tool use/result; signed thinking/redacted-thinking blocks needed by a tool turn | unsigned/malformed thinking, `is_error`, non-text tool results, native cache/beta block semantics, source stop sequence and non-common terminal reasons | Signed thinking is wrapped in a gateway-prefixed opaque `reasoning.encrypted_content` item; other Anthropic block/lifecycle identity is omitted |
...
After model-catalog resolution, the gateway looks up an immutable embedded conversion profile by the exact `(upstream_model, upstream_api)` pair. [GitHub's supported-models page](https://docs.github.com/en/copilot/reference/ai-models/supported-models) is the authority for current Copilot model membership, release status, and advertised configurable reasoning. The dynamic Copilot `/models` result and `model_catalog_json` remain the authority for account availability and exact upstream IDs/APIs; [the client compatibility matrix](../compatibility/matrix.json) remains the authority for client/version contracts. The embedded [conversion matrix](../internal/modelcatalog/conversion_matrix.json) neither exposes a model nor grants a client capability.
...
For Responses -> Messages, an exact model profile may also declare `adaptive_by_default`. Sonnet 5 uses adaptive thinking by default while Opus 4.8 does not. Current reference evidence treats Opus 5 like Opus 4.8 for this distinction: it supports explicit adaptive `xhigh`/`max`, but is not enabled by default until a direct Copilot probe proves otherwise. A first Sonnet 5 request therefore projects `thinking.type=adaptive`; explicit Responses effort is clamped to the model's declared Anthropic effort levels, and `temperature`/`top_p` are removed because Anthropic does not accept those sampling controls with thinking. A tool-result continuation enables adaptive thinking only when the matching signed thinking history was restored; without that history it sends `thinking.type=disabled` rather than constructing an invalid interleaved-thinking turn.
...
On the response side, Chat reasoning text can be retained for Chat or rebuilt as a Responses reasoning item. When a Chat upstream serves a Messages client, unsigned Chat `reasoning_content` has no safe Anthropic thinking representation: the response policy omits it and continues with final text/tool output instead of synthesizing an unsigned thinking block. For Anthropic -> Responses tool loops, a complete `thinking` block with a non-empty signature, or a complete redacted-thinking block, is serialized into `reasoning.encrypted_content` under the `ghcp-anthropic-thinking-v1:` prefix; the optional summary carries visible thinking text. When that exact envelope returns before a matching function call, the gateway strictly decodes it and restores the original block ahead of `tool_use`. The payload is base64-encoded transport state, not a confidentiality mechanism, and is never logged. Unknown prefixes, malformed envelopes, unsigned blocks, unrelated Responses reasoning, and thinking outside this tool-history bridge remain fail-closed. This does not imply a unified request-side reasoning level.
...
When Chat is routed to Copilot Responses, `temperature`, `top_p`, and `stop` are removed by the centralized direction policy and recorded in the normalized-field diagnostic. Copilot Responses has rejected these sampling controls in reproducible probes; the provider also removes them as a final serialization guard.
...
The direction policy omits `temperature` and `top_p`, which the Copilot Responses endpoint does not accept, and records their JSON paths in the normalized-field diagnostic. The provider repeats this deletion as a final serialization guard. The other listed parameters are written to the upstream body with the same name when their selected direction supports them.
...
Cross-protocol resume normalization is narrower than wire-mode detection. Only an authenticated, enabled `codex-candidate` profile combined with a syntactically valid `codex_exec/<major>.<minor>.<patch>` User-Agent may discard Responses reasoning-history items on a Chat/Messages route. On a Messages route it also consumes the private Lite marker and prepends `(continuing the conversation)` when compacted history starts with an assistant turn. A forged Lite header, a Codex User-Agent without the profile, or another profile does not receive this downgrade. This adaptation supports text/stream resume continuity only: it does not enable matrix-disabled `function_tools`, `compact`, or WebSocket capabilities, and Anthropic thinking/signature responses still fail closed because no verified round-trip encoding exists here.
...
Conversions: when the upstream is also Responses, typed message, reasoning, function/custom/tool-search call and output items are rebuilt in their original order with identity/status fields. The compatibility adapter wraps unsupported custom/tool-search/namespace tools as functions. For cross-protocol use, direct `input_text`, `input_image`, `text`, and `image_url` items are grouped into a user message; function calls become assistant tool calls and outputs become tool messages. Non-conversation items never create synthetic empty user messages, and adjacent assistant text/function calls are grouped into one canonical tool turn. Other item families are rejected by the semantic gate unless the verified Codex resume downgrade above explicitly removes them.
...
`stop_sequences` is renamed to `stop`. `thinking` and `output_config` remain native on Messages; on an OpenAI target their generation-depth intent is projected only when the exact resolved model profile declares `reasoning.effort` or `reasoning_effort`, then the source fields are removed. An unprofiled target removes them without injecting a provider dialect. `context_management` is removed on both OpenAI targets. `tool_choice.disable_parallel_tool_use:true` maps to OpenAI `parallel_tool_calls:false`; in the reverse direction an explicit OpenAI `false` maps to Anthropic `tool_choice.disable_parallel_tool_use:true` when tools exist. For streaming native Messages responses, an object-valued `context_management` field on the final `message_delta` event is preserved at the same location; a non-object value or the field on another event remains an upstream protocol error. Anthropic `metadata` remains an upstream body parameter; binding pools may also read `metadata.user_id` / `metadata.user` as `user_id` and `metadata.session_id` / `metadata.session` as `session_id`.

The typed parser recognizes `thinking.display` values `omitted` and `summarized`. `summarized` is a Cherry Studio compatibility hint: it is retained only long enough for the selected Chat/Responses direction policy to remove the complete Anthropic-only `thinking` object and report `$.thinking` as normalized. On native Copilot Messages, an exact model profile validates thinking type, budget support, display values, and effort levels. A request without a profile retains the conservative fallback that permits only `display:"omitted"`; `summarized` remains rejected at the provider semantic boundary, and other display values remain parser errors.

Native preservation includes `text`, `image`, `tool_use`, `tool_result`, tool-result `is_error`, `thinking` with signature, `redacted_thinking`, `cache_control`, and `context_management`. Cross-protocol conversion projects ordinary tool use/result, images, and text; thinking/redacted-thinking history, `is_error`, and Anthropic cache metadata are removed with path diagnostics because the OpenAI projections already omit them. Image-bearing tool-result content remains fail closed until a verified multimodal function-output contract exists.

Rejected or normalized: unknown optional fields in recognized message, block, image-source, function-tool, tool-choice, thinking, output-config, and cache-control envelopes are removed; unknown block/tool-choice/image-source discriminators and invalid core fields remain rejected. A boolean Cherry Studio `tools[].eager_input_streaming` hint is discarded, while a non-boolean value still returns a path-specific `400`. The `"[Circular]"` serialization placeholder is removed only in `cache_control` positions and does not affect the same string in message text or tool schemas. Empty/whitespace text blocks are omitted before Anthropic serialization; a request with no remaining message content is rejected locally instead of becoming an upstream 400. Converted parallel function calls/results are merged into one assistant tool-use turn followed by one user tool-result turn, with tool results ordered before ordinary user text. Image validation is the same as Chat Completions. Native Messages does not forward arbitrary headers or raw JSON fields.
...
| `ModelConversion` | none | none | none | Embedded exact-model parameter profile attached after catalog resolution; not client input or runtime availability |
...
Usage is normalized to input/output/cached/reasoning tokens, AI credits, and cost estimates. Canonical `InputTokens` is inclusive: fresh input plus cache reads plus cache writes. Chat and Responses therefore expose that inclusive total and their cached-token detail, while Anthropic output subtracts both cache buckets for `input_tokens` and emits them separately as `cache_read_input_tokens` / `cache_creation_input_tokens`. This keeps `fresh + cache_read + cache_creation == canonical input` across all nine routes. The internal source is explicitly `upstream`, `estimated`, or `missing` and is persisted in both the provider-attempt journal and usage ledger. Missing upstream usage is omitted from client JSON instead of being fabricated as a zero object; an explicitly present all-zero upstream object remains present. A missing source retains the maximum budget reservation rather than settling it as zero. Responses stream usage includes OpenAI-style `input_tokens_details.cached_tokens` and `output_tokens_details.reasoning_tokens` as well as gateway cost/cache extensions.

Streaming correctness rules are explicit: Chat `[DONE]` is accepted only after a validated non-empty final `finish_reason`, while EOF may complete only after that same terminal evidence; Responses upstream must emit `response.completed` or `response.incomplete`, and output/content/item done events are never response-level terminal evidence; native Messages succeeds only after `message_stop`. Malformed Chat SSE JSON terminates the stream, and Messages validates every content-block index and its start/delta/stop type lifecycle before accepting `message_stop`. Same-protocol Responses preserves upstream response IDs and output/content/summary indexes. GitHub Copilot may rotate `item_id` on every frame while retaining one `output_index`; after an explicit `response.output_item.added`, the gateway treats that index and item kind as authoritative, keeps the first item ID, and aliases later frame IDs so tokens do not become separate output items. Distinct implicit items and kind collisions remain separate or fail closed. Every downstream Responses JSON event receives a fresh monotonically increasing `sequence_number`; terminal text snapshots remain in their standard done/completed events but are never replayed as text deltas. Same-protocol Messages preserves block indexes, thinking signatures, tool IDs, stop reason, `stop_sequence`, and usage. Non-streaming Chat, Responses, and Messages bodies must contain exactly one JSON value followed by EOF. An incomplete Responses terminal is returned to the client and recorded as `incomplete`, not `success`. Failed events, read errors, malformed frames, trailing JSON, and premature EOF become protocol-level failures rather than fabricated successful completion.
...
- Anthropic `stop_sequences` is renamed to upstream `stop`; a returned `stop_sequence` value is omitted on Chat/Responses output while the equivalent terminal reason is preserved.
...
- Cross-protocol stream conversion rebuilds compatible shapes and declares source lifecycle/block/item identity omission as a known shape normalization. Anthropic cache metadata and equivalent stop-sequence values may be omitted; refusals, reasoning/thinking signatures without a verified bridge, non-final message phases, tool namespaces/types, and other non-representable semantics fail closed before their payload is written.