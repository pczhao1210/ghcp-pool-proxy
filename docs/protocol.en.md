...
On the response side, Chat reasoning text can be retained for Chat or rebuilt as a Responses reasoning item. When a Chat upstream serves a Messages client, unsigned Chat `reasoning_content` has no safe Anthropic thinking representation: the response policy omits it and continues with final text/tool output instead of synthesizing an unsigned thinking block. Only on the authorized Responses-entry -> Anthropic-upstream route, the returning Anthropic response may wrap a complete signed `thinking` block or complete redacted-thinking block into `reasoning.encrypted_content` under the `ghcp-anthropic-thinking-v1:` prefix; the optional summary carries visible thinking text. When that exact envelope returns before a matching function call on the same route, the gateway strictly decodes it and restores the original block ahead of `tool_use`. Native Messages remains byte-shape semantic passthrough, and Messages -> Responses retains its prior fail-closed thinking/signature behavior. The payload is base64-encoded transport state, not a confidentiality mechanism, and is never logged. Unknown prefixes, malformed envelopes, unsigned blocks, unrelated Responses reasoning, and thinking outside this route-local tool-history bridge remain fail-closed. This does not imply a unified request-side reasoning level.
